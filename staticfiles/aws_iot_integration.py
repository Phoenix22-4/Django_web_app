...
class AWSIoTManager:
    ...
    def get_device_shadow(self, device_id):
        """Get device shadow from AWS IoT, create if doesn't exist"""
        try:
            if not self.iot_client:
                return None
            ...
            if error_code == 'ResourceNotFoundException':
                logger.info(f"Device shadow not found for {device_id} - ensuring device exists and creating shadow")
                # First ensure the device thing exists
                if not self.ensure_device_thing_exists(device_id):
                    logger.error(f"Failed to ensure device thing exists for {device_id}")
                    return None
                
                # Create initial shadow with default state
                initial_state = {
                    "pump_status": False,
                    "pump_current": 0.0,
                    "overhead_level": 0,
                ...
                if self.create_device_shadow(device_id, initial_state):
                    logger.info(f"Successfully created device shadow for {device_id}")
                    return initial_state
                else:
                    logger.error(f"Failed to create device shadow for {device_id}")
                    return None
            else:
    ...
    def ensure_device_thing_exists(self, device_id):
        """Ensure the device thing exists in AWS IoT Core"""
        try:
            if not self.iot_core_client:
                logger.warning("AWS IoT Core client not available")
                return False
                
            # Try to describe the thing
            self.iot_core_client.describe_thing(thingName=device_id)
            logger.info(f"Device thing {device_id} already exists")
            return True
            
        except ClientError as e:
            error_code = e.response['Error']['Code']
            if error_code == 'ResourceNotFoundException':
                logger.info(f"Device thing {device_id} not found - creating it")
                try:
                    # Create the thing
                    self.iot_core_client.create_thing(
                        thingName=device_id,
                        thingTypeName='AquaGuardDevice',
                        attributePayload={
                            'attributes': {
                                'device_type': 'water_monitor',
                                'created_by': 'aquaguard_system'
                            }
                        }
                    )
                    logger.info(f"Successfully created device thing {device_id}")
                    return True
                except Exception as create_error:
                    logger.error(f"Error creating device thing {device_id}: {create_error}")
                    return False
            else:
                logger.error(f"AWS IoT error checking device thing {device_id}: {e}")
                return False
        except Exception as e:
            logger.error(f"Error ensuring device thing exists for {device_id}: {e}")
            return False
